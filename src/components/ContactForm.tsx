...
import { NavIconMail } from "@/components/nav/NavbarSketchIcons";

/** Avión de papel — mismo path que `public/svg/paper-plane-svgrepo-com.svg`, `currentColor` para el botón */
function IconPaperPlane({ className }: { className?: string }) {
  return (
    <svg
      className={className}
      viewBox="0 -10.03 112 112"
...
function ContactForm() {
  ...
  if (isSubmitted) {
    return (
      <div className="rounded-2xl bg-green-950/25 border border-green-500/40 p-8 sm:p-10 text-center">
        <CheckCircle
          className="w-14 h-14 sm:w-16 sm:h-16 text-green-500 mx-auto mb-4"
          aria-hidden
        />
        ...
        <button
          ...
          className="btn-punk inline-flex items-center justify-center gap-2"
        >
          <NavIconMail className="w-5 h-5 shrink-0" aria-hidden />
          {t("sendAnother")}
        </button>
      </div>
    );
  ...
  const fieldBase =
    "w-full rounded-xl text-white placeholder-zinc-500 focus:outline-none focus:ring-2 transition-colors duration-200 min-h-[48px]";
  const fieldOk =
    "bg-zinc-950/80 border border-zinc-700 focus:ring-red-600 focus:border-red-600";
  const fieldErr = "border-red-500 focus:ring-red-500 bg-zinc-950/80";
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-5 sm:gap-6 order-1">
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            autoComplete="name"
            className={`${fieldBase} pl-12 pr-4 py-3 ${
              errors.name ? fieldErr : fieldOk
            }`}
      ...
      <div>
        ...
        <select
          ...
          onChange={handleChange}
          className={`${fieldBase} px-4 py-3 appearance-none bg-zinc-950/80 cursor-pointer ${
            errors.subject ? fieldErr : fieldOk
          }`}
        >
      ...
      <div>
        ...
        <div className="relative">
          <MessageSquare
            className="absolute left-3.5 top-3.5 w-5 h-5 text-zinc-500 pointer-events-none"
            aria-hidden
          />
          <textarea
            id="message"
            ...
            onChange={handleChange}
            className={`w-full min-h-[160px] pl-12 pr-4 py-3 rounded-xl text-white placeholder-zinc-500 focus:outline-none focus:ring-2 transition-colors duration-200 resize-y ${
      ...
      <button
        ...
        className={`order-2 w-full min-h-[52px] btn-punk inline-flex items-center justify-center gap-2 ${
      ...
      >
        {isSubmitting ? (
          ...
          <>
            <IconPaperPlane className="w-5 h-5 shrink-0" />
            <span>{t("send")}</span>
          </>
        )}
      </button>

      <div className="order-3 rounded-xl border border-zinc-800/90 bg-black/35 p-4 sm:p-5">
        <h4 className="text-white font-bold text-sm sm:text-base mb-2">
          {t("privacyTitle")}
        </h4>
        <p className="text-zinc-400 text-sm leading-relaxed">{t("privacyText")}</p>
      </div>
    </form>
  );
}
